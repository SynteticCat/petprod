...
export default function EventsTable() {
    ...
    ]);  // TODO: заглушка до коннекта с серваком
    ...
    React.useEffect(() => {
        fetch("https://reqres.in/api/users?page=2")
          ...
          .then(
            (result) => {
              setIsLoaded(true);
              // TODO: как бэк завертится, запушим без заглушек
              // setRows(result);
            },
            (error) => {
              setIsLoaded(true);
    ...
    if (error) {
        return 'Error on server';  // TODO: в отдельную компоненту
    } else if (!isLoaded) {
        return 'Loading...';  // TODO: в отдельную компоненту
    } else {
        return (
